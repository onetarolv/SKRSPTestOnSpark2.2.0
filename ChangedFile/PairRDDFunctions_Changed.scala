...
class PairRDDFunctions[K, V](self: RDD[(K, V)])
  ...
  def reduceByKey(func: (V, V) => V): RDD[(K, V)] = self.withScope {
      val default = defaultPartitioner(self)
      reduceByKey(default, func)
  }
  ...
  def groupByKey(numPartitions: Int): RDD[(K, Iterable[V])] = self.withScope {
    val partitioner = self.conf.get("spark.partitioner.class","hash")
    if(partitioner.equals("range")){
      //DefRange
      val range = new RangePartitioner(numPartitions, self)
      groupByKey(range)
    }else if(partitioner.equals("reshash")){
      //KRHP-R
      val reshash = new BalanceHashPartitionerByReservoir(numPartitions,self)
      groupByKey(reshash)
    }else if(partitioner.equals("rejhash")){
      //KRHP-S
      val rejhash = new BalanceHashPartitionerByReservoir(numPartitions,self)
      groupByKey(rejhash)
    }else {
      //DefHash
      val hash = new HashPartitioner(numPartitions)
      groupByKey(hash)
    }
  ...
  def groupByKey(): RDD[(K, Iterable[V])] = self.withScope {
    val partitioner = self.conf.get("spark.partitioner.class","hash")
    val partitions = self.conf.getInt("spark.default.parallelism",self.getNumPartitions)
    if(partitioner.equals("range")){
      //DefRange
      val range = new RangePartitioner(partitions, self)
      groupByKey(range)
    }else if(partitioner.equals("reshash")){
      //KRHP-R
      val reshash = new BalanceHashPartitionerByReservoir(partitions,self)
      groupByKey(reshash)
    }else if(partitioner.equals("rejhash")){
      //KRHP-S
      val rejhash = new BalanceHashPartitionerByRejection(partitions,self)
      groupByKey(rejhash)
    }else {
      //DefHash
      val default = defaultPartitioner(self)
  ...
  def join[W](other: RDD[(K, W)]): RDD[(K, (V, W))] = self.withScope {
   ...
   if(partitioner.equals("range")){
     ...
     //DefHash
      val default = defaultPartitioner(self,other)
      join(other, default)
    }
