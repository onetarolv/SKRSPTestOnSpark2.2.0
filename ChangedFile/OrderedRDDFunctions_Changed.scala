...
class OrderedRDDFunctions[K : Ordering : ClassTag,
  ...
  {
    val partitioner = self.conf.get("spark.partitioner.class","range")
    val partitions = self.conf.getInt("spark.default.parallelism",self.getNumPartitions)
    if(partitioner.equals("resrange")){
      // KSRP-R
      val resrange = new SplitRangePartitionerByReservoir(numPartitions,self)
      new ShuffledRDD[K, V, V](self, resrange)
        .setKeyOrdering(if (ascending) ordering else ordering.reverse)
    }else if(partitioner.equals("rejrange")){
      //SKRP-S
      val rejrange = new SplitRangePartitionerByRejection(numPartitions,self)
      new ShuffledRDD[K, V, V](self, rejrange)
        .setKeyOrdering(if (ascending) ordering else ordering.reverse)
    }else {
      //DefRange
      val range = new RangePartitioner(partitions, self)
      new ShuffledRDD[K, V, V](self, range)
        .setKeyOrdering(if (ascending) ordering else ordering.reverse)
    }
  }
